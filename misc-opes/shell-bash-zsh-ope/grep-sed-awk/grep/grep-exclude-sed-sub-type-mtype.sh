...
FileNames=$(grep --exclude-dir="*.git*"  -e  $MailOld  . -R -l |  grep -e "pp$")
...
do
    ...
    #sed  -n  "s/$MailOld/$MailNew/g"  $file
    #sed -n  "/$MailOld/p"  $file       # -n 只输出匹配行
    sed -i  "/$MailOld/s/$MailOld_t/$MailNew/g"  $file      # 用 $MainOld 搜索, 替换$MainOld_t: 因为[^m]type_t 比 type_t 多了一个字符
done
